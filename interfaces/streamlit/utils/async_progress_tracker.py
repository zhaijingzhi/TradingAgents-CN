                ...
                for key in keys:
                    try:
                        ...
                        if data:
                            ...
                            history[analysis_id] = progress_data
                    except Exception as e:
                        logger.debug(f"📊 [历史记录] 解析Redis数据失败 {key}: {e}")
                        continue

                if history:
                    logger.info(f"📊 [历史记录] 从Redis获取到 {len(history)} 条记录")
                    return history

            except Exception as e:
                logger.debug(f"📊 [历史记录] Redis查询失败: {e}")
        ...
        if data_dir.exists():
            ...
            for file_path in progress_files:
                try:
                    ...
                    logger.debug(f"📊 [历史记录] 读取文件失败 {file_path}: {e}")
                    continue

            if history:
                logger.info(f"📊 [历史记录] 从文件获取到 {len(history)} 条记录")
